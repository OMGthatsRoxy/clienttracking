...
export default function MonthlyIncomePage() {
  ...
  // 本月进账总额
  const monthlyIncome = monthlyPackages.reduce((total, pkg) => total + (pkg.totalAmount || 0), 0);
      ...
      <div style={cardStyle}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: "clamp(10px, 2vw, 12px)", opacity: 0.9 }}>
            {currentMonth} {t('monthlyTotalIncome')}
          </div>
          <div style={{ fontSize: "clamp(8px, 1.5vw, 10px)", opacity: 0.8, marginTop: 4 }}>
            {t('totalPackages').replace('{count}', monthlyPackages.length.toString())}
          </div>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        {monthlyPackages.length === 0 ? (
          ...
          <div style={{ 
            ...
            {monthlyPackages
              .sort((a, b) => (b.totalAmount || 0) - (a.totalAmount || 0)) // 按金额降序排列
              .map((pkg) => {
